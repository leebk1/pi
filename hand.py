...
def send_trigger():
    ...
    TX_PIN = 4  # Chân GPIO dùng để phát tín hiệu
    pi = pigpio.pi()
    if not pi.connected:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if is_five_fingers_open(hand_landmarks):
                    print("Fingers opened detected")
                    found,processed_frame = process_frame(image)
                    # Get the text and boxes to be drawn based on the processed frame
                    display_frame = draw_results(processed_frame)
                    if found and (start - time.time() > 30 or not_init):
                        send_trigger()
                        print("owner detected, sleep for 10s")
                        start = time.time()
                        not_init = True
                        time.sleep(10)
                        sys.exit(1)
